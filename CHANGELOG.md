...
- Various cog options are availabe to make settings of both blocks quicker to apply and provide flexibility for those Mobirise builder users who are not so comfortable editing HTML & CSS. 
  ...
  - <b>BLOCK 2: "MobiGary Configurable Mobirise Bootstrap Floating Contact Form" for witsec Mailform Extension v12.x - Block v1.9.</b><br> 
    ...
    - Added new gear icon setting "Border box-shadow Color" to allow uses to select the box-shadow color (if "border box-shadow" enabled). It was set in CSS as black previously.<br>
![alt text](https://github.com/MobiGary/MobiriseM4blocks/blob/main/assets/images/border-box-shadow-col-v1.9.jpg)

    - Added 3 new gear icon settings "Field Border Thickness", "Field Border Hover Color" & "Field Border Hover Thickness". If "field border" is enabled this will allow the additional settings to be configured:
      ...
      3. field border thickness when the field is hovered over or selected (between 1px to 5px in 0.5 increments). If you do not require the field hover/selection to be different than the regular field border, use same color and thickness settings for both.
